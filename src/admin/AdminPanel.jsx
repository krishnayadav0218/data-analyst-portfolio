...
import {
  ...
  ChevronUp, ChevronDown, UploadCloud, FileText
} from 'lucide-react';
import './AdminPanel.css';
import { blocksToText, textToBlocks } from '../utils/blogContent';
import { profileData } from '../profileData';
import AdminLoginScene3D from './AdminLoginScene3D.jsx';
...
function LoginScreen({ onLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        if (res.status === 401) {
          flashError(data.error || 'Incorrect password.');
          setLoading(false);
          return;
        }
      }
    ...
    try {
      ...
      flashError('Could not reach the admin backend. Check your deployment and environment variables.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="admin-login-screen">
      ...
      >
        <span className="admin-login-brand-mark">KY</span>
        <span>Portfolio Admin</span>
      </motion.div>

      <motion.div
        className="admin-login-badges"
        initial={{ opacity: 0, y: 12 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5, delay: 0.15 }}
      >
        <span><CheckCircle2 size={13} /> Live-editable content</span>
        <span><BarChart3 size={13} /> Full CMS control</span>
        <span><Cloud size={13} /> Auto-deployed changes</span>
      </motion.div>

      <motion.form
        className="admin-login-card tilt-card"
      ...
      >
        ...
        <h1>Welcome back</h1>
        <p className="admin-login-sub">Sign in to manage projects, resume, and every section of your portfolio.</p>
        ...
        <motion.button
          type="submit"
          className="admin-btn-primary admin-btn-shimmer"
          disabled={loading}
          whileHover={{ scale: 1.03, y: -2 }}
          whileTap={{ scale: 0.95 }}
          transition={{ type: 'spring', stiffness: 400, damping: 15 }}
        >
          <span className="shimmer-ring" aria-hidden="true"></span>
          {loading ? <Loader2 size={16} className="spin" /> : <Lock size={16} />}
          {loading ? 'Checking…' : 'Sign In'}
        </motion.button>

        <p className="admin-login-footnote">Only Krishna has access — this panel is not public.</p>
      </motion.form>
    </div>
  );
}
...
function ProfileTab({ profile, onSave }) {
  ...
  const updateSocialLink = (i, key, value) =>
    setForm((f) => ({
      ...f,
      socialLinks: (f.socialLinks || []).map((s, idx) => (idx === i ? { ...s, [key]: value } : s)),
    }));

  // ---- Resume: drag-and-drop or click-to-upload, straight from any device ----
  // The PDF is read in the browser and stored as a base64 data URL in
  // `resumeUrl`, so it saves through the same profile-save flow as every
  // other field and works from phones, tablets, or desktops with no
  // separate file server needed.
  // Kept conservative (not the full 8MB some hosts allow) so uploads still
  // go through on the Vercel/Netlify GitHub-commit deploy mode, where the
  // serverless function has a hard ~4.5MB request-body ceiling once the
  ...
  const handleResumeFile = (file) => {
    ...
    if (file.size > MAX_RESUME_MB * 1024 * 1024) {
      setResumeError(`That file is too large — please keep it under ${MAX_RESUME_MB}MB.`);
      return;
    }
    const reader = new FileReader();
    reader.onload = () => {
      set('resumeUrl', reader.result);
  ...
  const onResumeDragOver = (event) => {
    event.preventDefault();
    setDragActive(true);
  };
  const onResumeDragLeave = () => setDragActive(false);
  ...
  const isUploadedResume = typeof form.resumeUrl === 'string' && form.resumeUrl.startsWith('data:');
  ...
  return (
    <Section title="Profile" subtitle="Your name, headline, and contact details shown across the site.">
      ...
      <h3 className="admin-subhead">Resume</h3>
      <div
        className={`admin-upload-zone tilt-card${dragActive ? ' drag-active' : ''}`}
      ...
      >
        ...
        />
        <UploadCloud size={26} />
        <p className="admin-upload-title">Drag & drop your resume PDF here, or click to browse</p>
        <p className="admin-upload-hint">Works from any device — phone, tablet, or desktop. PDF only, up to {MAX_RESUME_MB}MB.</p>
      </div>
      {resumeError && <p className="admin-upload-error">{resumeError}</p>}
      {form.resumeUrl ? (
        <div className="admin-upload-preview">
          <FileText size={16} />
          <span>{form.resumeFileName || (isUploadedResume ? 'Uploaded resume.pdf' : form.resumeUrl)}</span>
          <a href={form.resumeUrl} target="_blank" rel="noreferrer">View</a>
          <button className="admin-icon-btn danger" onClick={removeResume} title="Remove resume">
            <Trash2 size={15} />
          </button>
        </div>
      ) : null}
      <Field
        label="Or paste a resume link instead (e.g. Google Drive, Dropbox share link)"
      ...
      <h3 className="admin-subhead">More Social Links</h3>
      {socialLinks.map((link, i) => (
        <div className="admin-card tilt-card" key={i}>
          <div className="admin-card-row">
            <Field
              label="Label (e.g. Twitter, Portfolio, YouTube)"
              value={link.label}
              onChange={(v) => updateSocialLink(i, 'label', v)}
            />
            <Field label="URL" value={link.url} onChange={(v) => updateSocialLink(i, 'url', v)} />
            <button
      ...
      <button className="admin-btn-ghost" onClick={addSocialLink}>
...
function ProjectsTab({ projects, onSave }) {
  ...
  return (
    <Section title="Projects" subtitle="Case studies and project cards shown on your site. Use the arrows to reorder how they appear.">
      {list.map((p, i) => (
        <div className="admin-card tilt-card" key={i}>
          <div className="admin-card-row">
          ...
          <Field
            label="Cover Image URL (e.g. an Unsplash/Pexels photo link — shown at the top of the project card)"
            value={p.coverImage}
            onChange={(v) => update(i, 'coverImage', v)}
          />
          <Field
...
function MessagesTab({ messages, onDelete }) {
  return (
    <Section title="Contact Messages" subtitle="Messages submitted through your site's contact form.">
      ...
      {messages.map((m) => (
        <div className="admin-card tilt-card" key={m.id}>
          <div className="admin-card-row">
            <div>

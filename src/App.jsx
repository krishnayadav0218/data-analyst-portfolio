...
import RotatingRole from './RotatingRole.jsx';
...
function App() {
  ...
  // ---- Mobile nav toggle, scroll-spy active link, and top scroll progress bar ----
  const [navOpen, setNavOpen] = useState(false);
  const [activeSection, setActiveSection] = useState('home');
  const [scrollProgress, setScrollProgress] = useState(0);
  const NAV_SECTION_IDS = ['home', 'projects', 'blog', 'services', 'leads', 'skills', 'contact'];
  ...
  return (
    <main data-theme={theme}>
      <div className="scroll-progress-bar" style={{ transform: `scaleX(${scrollProgress / 100})` }}></div>
      <nav className="topbar" aria-label="Portfolio navigation">
        <div className="topbar-inner">
          ...
          <div className={`nav-links${navOpen ? ' open' : ''}`}>
            ...
            <a href="#leads" className={activeSection === 'leads' ? 'active' : ''}>Leads</a>
      ...
      <section className="hero-section hero3d-wrap" id="home">
        ...
        <div className="hero-copy">
          ...
          <p className="role-pill">
            <RotatingRole text={profile.role} />
          </p>
          ...
          <div className="service-grid">
            {profile.services.map((service) => (
              <article className="service-card tilt-card reveal" key={service.title}>
                ...
                <h3>{service.title}</h3>
                <p>{service.detail}</p>
                {service.priceRange && <span className="service-price">{service.priceRange}</span>}
              </article>
            ))}
          </div>
      ...
      <section className="section split matrix-section-wrap" id="skills">
        ...
        <aside className="credential-panel tilt-card reveal">
          ...
          {githubUsername && (
            <>
              <h3>
                <GitBranch size={20} /> GitHub activity
              </h3>
            ...
            <article className="testimonial-card tilt-card reveal" key={testimonial.name}>
              <div className="testimonial-stars" aria-label="5 out of 5 stars">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} size={15} fill="#d9a441" stroke="#d9a441" />
                ))}
              </div>
      ...
      <section className="section contact-section" id="contact">
        <div className="contact-copy">
          ...
          <div className="social-actions">
            <a
              className="book-call-cta"
              href={profile.calendlyUrl || '#leads'}
              target={profile.calendlyUrl ? '_blank' : undefined}
              rel={profile.calendlyUrl ? 'noreferrer' : undefined}
            >
